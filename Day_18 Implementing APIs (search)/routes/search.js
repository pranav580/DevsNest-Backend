...
router.get("/trgm", (req, res) => {
    ...
    sequelize
        .query("CREATE EXTENSIONS IF NOT EXISTS pg_trgm;")
        .then(() => {
            sequelize
                .query("select * from pg_extensions where extname='pg_trgm';")
                .then(() => {
                    Artist.findAll({
                        attributes: {
                            include: [
                        ...
                        where: [
                            sequelize.where(
                                sequelize.fn("similarity", sequelize.close("Name"), req.query.q),
                                { [Op.gt]: 0.2 }
...
router.get("/sound", async (req, res) => {
    try {
        //soundex will create a string of 4
...
router.get("/distance", async (req, res) => {
    try {
        //Levenshtein distance between two strings is the minimum number of edits required to transform one string to other.
        const extension = await sequelize.query(
            "CREATE EXTENSION IF NOT EXIST fuzzystrmatch;"
        );
        const artist = await sequelize.query(`SELECT
